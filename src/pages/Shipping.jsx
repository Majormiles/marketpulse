...
const Shipping = () => {
    ...
    useEffect(() => {
        ...
        if (savedShippingInfo) {
            setState(savedShippingInfo);
            setRes(true); // Automatically set `res` to true if info exists
        }
    }, []);

    // Save shipping info to localStorage whenever `state` updates
    useEffect(() => {
        if (res) {
    ...
    return (
        <div>
            <Headers />
            <section className='bg-[url("https://marketpulse-oxxa.onrender.com/images/banner/animated.gif")] h-[220px] mt-6 bg-cover bg-no-repeat relative bg-left'>
                                ...
                                <div className="bg-white p-6 shadow-sm rounded-md">
                                            ...
                                            <form onSubmit={save}>
                                                <div className='flex md:flex-col md:gap-2 w-full gap-5 text-slate-600'>
                                                    ...
                                                    <div className='flex flex-col gap-1 mb-2 w-full'>
                                                        ...
                                                        <input onChange={inputHandle} value={state.address} type="text" className='w-full px-3 py-2 border border-slate-200 outline-none focus:border-indigo-500 rounded-md' name='address' placeholder='House no / building / street / area' id='address' />
                                                    </div>
                                                </div>
                                                <div className='flex md:flex-col md:gap-2 w-full gap-5 text-slate-600'>
                                                    <div className='flex flex-col gap-1 mb-2 w-full'>
                                    ...
                                    }
                                    {
                                        res && <div className='flex flex-col gap-1'>
                                            ...
                                            <p className='text-slate-600 text-sm'>Email to majormyles20@gmail.com</p>
                                        </div>
                                    }
                                </div>
                                {
                                            ...
                                            p.products.map((pt, j) => <div key={i + 99} className='w-full flex flex-wrap'>
                                                ...
                                                <div className='flex justify-end w-5/12 sm:w-full sm:mt-3'>
                                                    <div className='pl-4 sm:pl-0'>
                                                        <h2 className='text-lg text-orange-500'>GH₵{pt.productInfo.price - Math.floor((pt.productInfo.price * pt.productInfo.discount) / 100)}</h2>
                                                        <p className='line-through'>GH₵{pt.productInfo.price}</p>
                            ...
                            <div className="pl-3 md-lg:pl-0">
                                ...
                                <div className='bg-white font-medium p-5 text-slate-600 flex flex-col gap-3'>
                                    ...
                                    <div className='flex justify-between items-center'>
                                        ...
                                        <span>GH₵{shipping_fee}</span>
                                    </div>
                                    <div className='flex justify-between items-center'>
                                        <span>Total Payment</span>
                                        <span>GH₵{price + shipping_fee}</span>
                                    </div>
                                    <div className='flex justify-between items-center'>
                                        <span>Total</span>
                                        <span>GH₵{price + shipping_fee}</span>
