...
const Details = () => {
    ...
    return (
        <div>
            <Headers />
            <div className='bg-[url("https://marketpulse-oxxa.onrender.com/images/banner/order.jpg")] h-[220px] mt-6 bg-cover bg-no-repeat relative bg-left'>
            ...
            <section>
                <div className='w-[75%] md:w-[80%] sm:w-[90%] lg:w-[90%] h-full mx-auto pb-16'>
                    <div className='grid grid-cols-2 md-lg:grid-cols-1 gap-8'>
                        <div>
                            <div className='p-4 border'>
                                <img className='h-[180px] w-[270px]' src={image ? image : product.images?.[0]} alt="" />
                            </div>
                            <div className='py-3'>
                                {
                                            ...
                                            product.images.map((img, i) => {
                                                return (
                                                    <div key={i} onClick={() => setImage(img)}>
                                                        <img className='h-[80px] cursor-pointer' src={img} alt="" />
                                                    </div>
                                                )
                                            })
                                        }
                        ...
                        <div className='flex flex-col gap-5'>
                            ...
                            <div className='text-2xl text-red-500 font-bold flex gap-3'>
                                {
                                    product.discount !== 0 ? <>
                                        <h2 className='line-through'>GH₵{product.price}</h2>
                                        <h2>GH₵{product.price - Math.floor((product.price * product.discount) / 100)} (-{product.discount}%)</h2>
                                    </> : <h2>Price : GH₵{product.price}</h2>
                                }
                            </div>
                            ...
                            <div className='flex gap-3 pb-10 border-b'>
                                {
                                    product.stock ? <>
                                        <div className="flex bg-slate-200 h-[50px] justify-center items-center text-xl">
                                            <div
                                                onClick={dec}
                                                className="w-[50px] sm:w-[30px] h-full flex justify-center items-center cursor-pointer bg-gray-300 hover:bg-gray-400"
                                            >
                                                -
                                            </div>
                                        ...
                                        <div>
                                            <button
                                                onClick={add_card}
                                                className="px-4 py-3 h-[50px] w-[130px] sm:w-[120px] cursor-pointer hover:shadow-lg hover:shadow-purple-500/40 bg-purple-500 text-white text-sm md:text-base lg:text-lg rounded-md"
                                            >
                                                Add To Cart
                                            </button>
                                        </div>
                            ...
                            <div className='flex gap-3'>
                                {
                                    product.stock ? <button onClick={buy} className='px-5 py-2 h-[40px] cursor-pointer hover:shadow-lg hover:shadow-emerald-500/40 bg-emerald-500 text-white'>Buy Now</button> : ""
                                }
                                <Link to={`/dashboard/chat/${product.sellerId}`} className='px-5 py-2 h-[40px] cursor-pointer hover:shadow-lg hover:shadow-lime-500/40 bg-lime-500 text-white block'>Chat Seller</Link>
                            </div>
                        </div>
                    </div>
                </div>
                                            ...
                                            return (
                                                <Link className='block'>
                                                    <div className='relative h-[160px]'>
                                                        <img className='w-[200px] h-[170px]' src={p.images[0]} />
                                                        {
                                                    ...
                                                    <div className='flex gap-2'>
                                                        <h2 className='text-[#6699ff] text-lg font-bold'>GH₵{p.price}</h2>
